...
Let us also assign unique values $1 \ldots n$ to individual symbols in $D$ and freely write $x$ instead of its assigned value.
...
If we only have one case we write

\[\ite{x}{i}{s}{t} := \caseelse{x}{i \mapsto s}{t}\]
...
consider the specification $\rho_\star \to \rho_0 \to \rho_{R_1} \to \cdots \to \rho_{R_k} \to \rho_1 \to \$$ where

\begin{align*}
\rho_\star & := \Big((\top \to \bullet) \to \$ \Big) \to \$\\
\rho_0 & := \1 \to (\top \to \bullet) \to \$\\
\rho_{ab \to cd} & := \1 \to (\top \to \bullet) \to \1\\
\rho_1 & := \1
\end{align*}

Inhabitants are necessary of shape $\lambda x_\star x_0 x_{R_1} \ldots x_{R_k} x_1.M$ where $M$ is TODO
...
F_\0 := &\lambda x^A. \lambda h^{A \to A}.h\,x\\
F_{ab \to cd} := &\lambda x^A. \lambda h^{A \to A}.h\,x\\\\
H_\star := &\lambda g^{(A \to A) \to A}.\lambda r_1 \ldots r_n. \caseelse{g\,\delta^\top_\bullet}{\$ \mapsto r_\$}{r_\bot}\\
H_\0 := &\lambda x^A. \lambda h^{A \to A}.\lambda r_1 \ldots r_n.\caseelse{h\,\pi_\top}{\bullet \mapsto \caseelse{x}{\1 \mapsto r_\$}{r_\bot}}{r_\bot}\\
H_{ab \to cd} := &\lambda x^A. \lambda h^{A \to A}.\lambda r_1 \ldots r_n.\caseelse{h\,\pi_\top}{\bullet \mapsto \caseelse{x}{\1 \mapsto r_\1}{r_\bot}}{r_\bot}\\
H_\1 := &\pi_\1
\end{align*}
...
\begin{align*}
&X\,F_\star\,F_\0\,F_{R_1} \ldots F_{R_k} = \lambda z.z\\
&X\,H_\star\,H_\0\,H_{R_1} \ldots H_{R_k}\,H_\1\,r_1\ldots r_n = r_\$
\end{align*}
