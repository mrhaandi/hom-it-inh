...
\begin{definition}~
For $m \in \bbN$ let $\calQ_m$ and $\calR_m$ be the smallest sets of terms satisfying the following rules.
\begin{itemize}
\item $z_\1 \in \calQ_m$
\item $M \in \calQ_m \implies (r_i\,p_j\,M) \in \calQ_m$
\item $M \in \calQ_m \implies (r_i\,(\lambda w.p_j\,w)\,M) \in \calQ_m$
\item $M \in \calQ_m \implies (z_\0\,N\,M) \in \calR_m$
\item $M \in \calR_{m+1} \implies (z_\star\,N\,(\lambda p_m.M)) \in \calR_m$
\end{itemize}
\end{definition}
...
H_\1 := &\pi_\1\\
\Gamma_m^\kappa := &\{ \begin{aligned}[t]
&p_1 : \kappa \to \kappa, \ldots, p_m : \kappa \to \kappa,\\
&z_\1 : \kappa, z_\star : (\kappa \to \kappa) \to ((\kappa \to \kappa) \to \kappa) \to \kappa, z_\0 : (\kappa \to \kappa) \to \kappa \to \kappa\\
&r_1 : (\kappa \to \kappa) \to \kappa \to \kappa, \ldots, r_k : (\kappa \to \kappa) \to \kappa \to \kappa\}
\end{aligned}
%\{p_1 : \kappa \to \kappa, \ldots, p_m : \kappa \to \kappa, z_\1 : \kappa, z_\star : (\kappa \to \kappa) \to ((\kappa \to \kappa) \to \kappa) \to \kappa, r_1 : (\kappa \to \kappa) \to \kappa \to \kappa, \ldots, r_k : (\kappa \to \kappa) \to \kappa \to \kappa\}
\end{align*}
\end{definition}
...
\item $\emptyset \vdash \delta_i : \Gamma_m^\kappa(p_i)$
\item $\emptyset \vdash H_\star : \Gamma_m^\kappa(z_\star)$
\item $\emptyset \vdash H_\0 : \Gamma_m^\kappa(z_\0)$
\item $\emptyset \vdash H_\1 : \Gamma_m^\kappa(z_\1)$
\item $\emptyset \vdash H_R : \Gamma_m^\kappa(r_i)$
\end{itemize}
\end{lemma}
...
%  ring2 m M.
If $M$ is in normal form, $\Gamma_\kappa \vdash M : \kappa$,\\
...
If $M$ is in normal form, $\Gamma_\kappa \vdash M : \kappa$,\\
$\begin{array}{lllllllll}
M[z_\star := \lambda h.\lambda g.g\,I, &z_\1 := u, &z_\0 := \lambda h.I, &r_1 := I, \ldots, &r_k := I, &p_1 := I] &=_\beta u,\\
M[z_\star := H_\star, &z_\1 := \pi_\1, &z_\0 := H_\0, &r_1 := H_R, \ldots &r_k := H_R, &p_1 := \delta_\bullet] &=_\beta \pi_{\$},
\end{array}$\\
then $M \in \calR_1$.
\end{lemma}
...
If $M$ is in normal form, $\emptyset \vdash M : ???$,\\
$\begin{array}{ll}
