...
\newcommand{\caseelse}[3]{\ensuremath{\textbf{case } {#1} \textbf{ of } \langle #2 \mid\mid #3 \rangle}}
...
\item $\delta_i := \lambda x.\lambda r_1 \ldots r_K.\caseelse{x}{\top \mapsto r_i}{r_\bot}$
\end{itemize}
...
\begin{definition}
\begin{align*}
H_\star := &\lambda h.\lambda g.\lambda r_1 \ldots r_K. \caseelse{g\,\delta_\bullet}{\$ \mapsto r_\$}{r_\bot}\\
H_\0 := &\lambda h.\lambda x. \lambda r_1 \ldots r_K.\caseelse{x}{\1 \mapsto r_\$}{r_\bot}\\
H_R := &\lambda h.\lambda x. \lambda r_1 \ldots r_K.\caseelse{h\,\pi_\top}{\bullet \mapsto \caseelse{x}{\1 \mapsto r_\1}{r_\bot} }{r_\bot}\\
H_\1 := &\pi_\1
\end{align*}
\end{definition}
...
$\begin{array}{lllllll}
M[z_\star := \lambda h.\lambda g.g\,I, &z_\1 := u, &z_\0 := \lambda h.I, &y_R := I, &x_p := I] &=_\beta u,\\
M[z_\star := H_\star, &z_\1 := \pi_\1, &z_\0 := H_\0, &y_R := H_R, &x_p := \delta_\bullet] &=_\beta \pi_{\1}
\end{array}$,\\
then $M = y_R\,N_1\,(y_R\,N_2\,(\ldots(y_R\,N_k\,z_\1)\ldots))$ for some $k \in \bbN$ such that $N_i = x_p$ or $N_i = \lambda w.x_p w$ for $i \in \{1, \ldots, k\}$.
\end{lemma}
...
$\begin{array}{lllllll}
M[z_\star := \lambda h.\lambda g.g\,I, &z_\1 := u, &z_\0 := \lambda h.I, &y_R := I, &x_p := I] &=_\beta u,\\
M[z_\star := H_\star, &z_\1 := \pi_\1, &z_\0 := H_\0, &y_R := H_R, &x_p := \delta_\bullet] &=_\beta \pi_{\$}
\end{array},$\\
then $M = z_\star\,N_1\,(\lambda x_p^1._\star\,N_2\,(\lambda x_p^2. \ldots (N)))$ for some $k \in \bbN$ and $N$ in special form.
\end{lemma}
...
\end{array}$\\
then $M = \lambda y_R^1 \ldots y_R^k.\lambda z_\0 z_\1 z_\star x_p^1.N$ for some $k \in \bbN$ such that $N$ is in special form.
\end{theorem}
