...
\title{Mechanized Undecidability of Higher-order beta-Matching}
...
\authorrunning{A. Dudenhefner} %mandatory. First: Use abbreviated first/middle names. Second (only in severe cases): Use first author plus 'et al.'

\Copyright{Andrej Dudenhefner} %mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/

\ccsdesc[500]{Theory of computation~Lambda calculus} %mandatory: Please choose ACM 2012 classifications from https://dl.acm.org/ccs/ccs_flat.cfm 

\keywords{lambda-calculus, simple types, undecidability, higher-order matching, mechanization, Coq} %mandatory; please add comma-separated list of keywords
...
\acknowledgements{I sincerely thank the anonymous reviewers for their constructive suggestions.}%optional
...
\begin{definition}[$\lambda$-Terms]\label{def:terms}
...
\begin{definition}[Simple Type System]\label{def:stlc}
...
\begin{example}\label{xmp:derive}~\\
...
Since simply typed terms are strongly normalizing and $\beta$-reduction is confluent~\cite{Barendregt13book}, the it suffices in Problem~\ref{prb:hom} to consider terms $F, M, N$ in normal form.
...
In the above Example~\ref{xmp:ad-hoc} we would like to exclude certain ad hoc solutions, in order to faithfully encode intended functional behavior.
...
\begin{example}[{\cite[Proposition~3.4]{Dowek01}}]\label{xmp:no-ad-hoc}
...
\begin{example}\label{xmp:no-ad-hoc-combo}
...
However, ad hoc solutions such as $\lambda f.\lambda s.\pi_3$ or $\lambda f.\lambda s.\lambda x y z.f\,s\,z\,z\,z$ from Example~\ref{xmp:ad-hoc} do not solve $F\,\mathsf{X} = N$ because such solutions contain too many $\lambda$-abstractions.
\end{example}

\begin{remark}\label{rem:typing-break}
Without the restriction $\emptyset \vdash M : (\kappa \to \kappa) \to \kappa \to \kappa$ in Example~\ref{xmp:no-ad-hoc} the term $M := \lambda f.\lambda s.f\,f\,s$ satisfies $M\,I\,u =_\beta u$.
However, Example~\ref{xmp:no-ad-hoc-combo} does not admit $M$ as a solution.
The present work relies on well-typedness, but might be adapted to an untyped scenario.
\end{remark}

\newpage

The following Remark~\ref{rem:eta-break} illustrates how the addition of $\eta$-reduction would make the technique from Example~\ref{xmp:no-ad-hoc-combo} (as well as Loader's approach) unsuitable.

\begin{remark}\label{rem:eta-break}
Example~\ref{xmp:no-ad-hoc} demonstrates how to restrict the number of abstractions in solutions.
However, in the presence of $\eta$-reduction (contextual closure of $\lambda x.f\,x \to_\eta f$) this does not work, as shown below.
Consider the terms $G := \lambda h.\lambda x y z.h\,y\,z\,x$, $\pi_1 = \lambda xyz.x$, $\pi_2 = \lambda xyz.y$, and $\pi_3 = \lambda xyz.z$ from Example~\ref{xmp:no-ad-hoc}.
...
&=_\beta& \lambda xyz.\pi_1\,z\,y\,x =_\beta \lambda xyz.z = \pi_{3}
\end{array}$\\
$\arraycolsep=1.4pt\begin{array}[t]{lclcl}
...
Joly shifts the technical burden to a particular $\lambda$-definability problem, which then is simpler to handle.
Instead, we avoid $\lambda$-definability altogether and use a rewriting problem in a restricted class of \emph{simple} semi-Thue systems as a starting point.
...
\begin{definition}[Simple Semi-Thue System]\label{def:ssts}
...
\end{theorem}
The following Example~\ref{xmp:ssts} illustrates a positive instance of Problem $\0^+ \Rightarrow^* \1^+$.
...
\begin{remark}
Problem $\0^+ \Rightarrow^* \1^+$ is used as a starting point in a refinement~\cite[Lemma~4.4]{DudenhefnerR19} of Urzyczyn's undecidability result for intersection type inhabitation~\cite{Urzyczyn09}.
Undecidability of Problem $\0^+ \Rightarrow^* \1^+$ is mechanized as part of Coq Library of Undecidability Proofs~\cite{CLUP20}, making it a good starting point for further mechanized undecidability results.
\end{remark}
