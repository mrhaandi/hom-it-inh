...
Equivalences $(1)$ -- $(4)$ witness the particular rewriting steps at positions $1$ -- $4$ (cf.~Example~\ref{xmp:H}).
\end{example}
...
The following Example~\ref{xmp:G_R} complements the previous Example~\ref{xmp:G_Q} and illustrates the intended meaning (word expansion and initialization) of a \enquote{well-formed} example term in $\calR_1$ under the substitution $S_G$.
...
We have $M_i \in \calR_i$ for $i \in \{1,2,3\}$, and the following $\beta$-equivalences hold, illustrating word expansion and initialization:
...
In combination with the previous Example~\ref{xmp:G_Q}, the term $M_1 \in \calR_1$ witnesses word expansion up to length $4$, followed by initialization with $\0$s, and rewriting to $\1$s via rule application.
\end{example}
...
the presented approach reducing Problem $\0^+ \Rightarrow_\mathfrak{R}^* \1^+$ to Higher-order $\beta$-matching 


TODO conclude with uniform proposition for three problems hom, inhabitation and definability
