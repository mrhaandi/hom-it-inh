...
The particular proof is by reduction from a definability problem for finite models of the simply typed $\lambda$-calculus.
...
Undecidability of higher-order $\beta$-unification was established by Huet~\cite{Huet73} in the 1970s, raising the question whether the corresponding $\beta$-matching problem~\cite{Huet75} (only the left-hand side term $M$ is subject to instantiation) is decidable\footnote{Dowerk~\cite{Dowek01} gives a comprehensive overview over unification and matching problems for the $\lambda$-calculus.}.
...
rather, we consider a rewriting problem in a restricted class of semi-Thue systems as a starting point.
This particular rewriting problem was identified by Urzyczyn~\cite{Urzyczyn09} in his undecidability proof for intersection type inhabitation.
As a result, the presented proof is simpler to verify in full detail and yields a concise mechanization.
The mechanization is incorporated into the existing Coq Library of Undecidability Proofs~\cite{CLUP20}, alongside the existing mechanization of undecidability of $\0^+ \Rightarrow^*_\frakR \1^+$.
%This entails a certified many-one reduction from the Turing machine Halting Problem to higher-order $\beta$-matching.

Urzyczyn's undecidability proof for intersection type inhabitation serves as the main inspiration for the novel approach in the present work.
Considering the relationship between intersection type inhabitation finite model theory~\cite{SalvatiMGB12}, the approach in the present work has an additional benefit.
In particular, the presented technique is uniformly applicable to higher-order $\beta$-matching,  intersection type inhabitation, and $\lambda$-definability.

TODO: say that ssts01 is related to turing machine computation \cite[Lemma~3.3]{DudenhefnerR19}
...
As shown in the above Example~\ref{xmp:ad-hoc}, we would like to exclude certain \enquote{ad-hoc} solutions.
The following Example~\ref{xmp:no-ad-hoc} shows how to restrict the shape of solutions.
...
Combining Example~\ref{xmp:ad-hoc} with Example~\ref{xmp:no-ad-hoc} we can formulate in the following Example~\ref{xmp:no-ad-hoc-combo} a matching instance which faithfully encodes functional behavior.
...
In the present work, we focus on a fragment, which can be identified identified by inspection of intersection types occurring in the undecidability of intersection type inhabitation~\cite{Urzyczyn09,DudenhefnerR19} and their relationship to finite model theory~\cite{SalvatiMGB12}.
