...
First, we identify the shape of terms, suitable to represent rewriting.
...
The subterm $r_i\,p_j$ (or, $r_i\,(\lambda w.p_j\,w)$) for $i \in \{1, \ldots, L\}$ and $j \in \{1, \ldots, m\}$ indicates an application of the rule $R_i$ at symbol positions $m$ and $m+1$.
...
Specifically, the subterm $(z_\star\,N\,(\lambda p_{m+1}.M))$ introduces an additional bound variable $p_{m+1}$ in order the argue about rule application at the end of the word of length $m+2$.
...
Similarly to Example~\ref{xmp:no-ad-hoc}, we formulate typed terms 

In the following definition 

\begin{definition}[Terms $H_\star, H_\0, H_R$]\label{def:H}
\begin{align*}
...
\begin{definition}[Substitutions $S_F$, $S_H$]~\\
$\begin{array}{lllllll}
...
$\begin{array}{ll}
S_F(M)[p_1 := I, \ldots, p_m := I] &=_\beta u,\\
