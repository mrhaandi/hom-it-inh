...
Let $\frakR  := \{\0\0 \Rightarrow \2\2, \0\2 \Rightarrow \1\1, \2\0 \Rightarrow \1\1\}$ over the alphabet $\{\0,\1,\2\}$.\\
We have $\0\0\0\0 \Rightarrow_\frakR \0\2\2\0 \Rightarrow_\frakR \1\1\2\0 \Rightarrow_\frakR \1\1\1\1$. Additionally, $\0^n \;{\not\Rightarrow}_\frakR^*\; \1^n$ for $n \in \{1, 2, 3\}$.
\end{example}
...
In this section we develop out main result (Theorem~\ref{thm:ssts_to_hom}): a reduction from the rewriting problem $\0^+ \Rightarrow_\mathfrak{R}^* \1^+$ to Higher-order $\beta$-Matching.
...
Our goal is to construct simply typed terms which capture the two main aspects of the rewriting problem $\0^+ \Rightarrow_\mathfrak{R}^* \1^+$: the search for a sufficiently long starting sequence of $\0$s, and the individual rewriting steps to the resulting sequence of $\1$s.
...
Third, for the restricted solutions shape we capture the functional properties of the rewriting problem $\0^+ \Rightarrow_\mathfrak{R}^* \1^+$.
...
A particular term, which we will use commonly is $\delta_i := \lambda x.\lambda s_\0 s_\1 \ldots s_\K s_\$ s_\bullet s_\top s_\bot.\caseelse{x}{\top \mapsto s_i}{s_\bot} \text{ for } i \in \calA$.
The key property of $\delta_i$ is
$\delta_i\,\pi_j =_\beta \begin{cases}
\pi_i & \text{if } j = \top\\
\pi_\bot & \text{if } j \neq \top
\end{cases}$ for $i, j \in \calA$.

As is usual, we abbreviate $\lambda x.x$ by $I$, and assign to it the simple type $\kappa \to \kappa$.

TODO $\emptyset \vdash \delta_i : \Gamma_m(p_j)$ for $i \in \calA$ and $j \in \{1, \ldots, m\}$

\subsection*{Syntactic Constrains}
First, we identify the shape of \enquote{well-formed} terms, suitable to represent rewriting.
...
The subterm $r_i\,p_j$ (or, $r_i\,(\lambda w.p_j\,w)$) for $i \in \{1, \ldots, L\}$ and $j \in \{1, \ldots, m\}$ indicates an application of the rule $R_i$ at position $m+1-j$.
...
Specifically, the subterm $(z_\star\,N\,(\lambda p_{m+1}.M))$ introduces an additional bound variable $p_{m+1}$ in order the argue about rule application in the longer word.
...
\begin{definition}[$\calQ_m$, $\calR_m$]\label{def:ring}
...
\begin{remark}
Terms in $\calR_1$ closely resemble candidate inhabitants in a refinement~\cite[Lemma~4.4]{DudenhefnerR19} of Urzyczyn's undecidability result for intersection type inhabitation~\cite{Urzyczyn09}.
\end{remark}
...
We introduce substitutions $S_F$ and $S_H$ (Definition~\ref{def:SF_SH}) acting on the term variables $\{z_\star, z_\1, z_\0, r_1, \ldots, r_L\}$, which occur in terms in $\calQ_m$ and $\calR_m$.
...
Let $N := r_1\,p_2\,(r_2\,p_3\,(r_3\,p_1\,z_\1))$ and
$M := z_\star\,p_1\,(\lambda p_2.z_\star\,p_2\,(\lambda p_3.z_\0\,p_3\,N))$.
The intended meaning of $M$ is that an initial word of length $2$ is expanded twice (using $z_\star$) to a word of length $4$ and initialized to $\0$s (using $z_\0$).
The introduced variables $p_2$ and $p_3$ are used to address positions in the longer word.
The intended meaning of $N$ is that the first rule (using $r_1$) is applied at position $2$ (using $p_2$), followed by the second rule at positions $1$ and third rule at positions $3$ accordingly.
...
In congruence with Theorem~\ref{thm:shape} the following $\beta$-equivalences hold:\\
$\begin{array}{ll}
...
The presented terms are functional programs which realize the intended meaning (Example~\ref{xmp:H}) of the \enquote{well-formed} terms in $\calQ_m$ and $\calR_m$.

\begin{definition}[Typed Terms $G_\star$, $G_\0$, $G_{ab \to cd}$, $G_i^j$]
\label{def:G}
\begin{align*}
...
G_{ab \to cd} := &\lambda h.\lambda x.\lambda s_\0 s_\1 \ldots s_\K s_\$ s_\bullet s_\top s_\bot.\case{h\,\pi_\top}{s_\bot\\
...
$S_G(M)[p_1 := \delta_\0] =_\beta \pi_\1$, and
...
Considering the general case $M \in \calR_{m'}$, by induction on the size of $M$ and exhaustive case analysis using Definition~\ref{def:ring}.
\end{proof}
...
For $m > 0$ let $a_1, \ldots, a_{m + 1} \in \{\0, \1, \ldots, \K\}$. If $M \in \calQ_m$ such that $\Gamma_m \vdash M : \kappa$,\\
...
The following Example~\ref{xmp:G} builds upon the previous Example~\ref{xmp:H} and illustrates the intended meaning of a \enquote{well-formed} term under the substitution $S_G$.

\begin{example}\label{xmp:G}
Assume $\frakR = \{\0\0 \Rightarrow \2\2, \0\2 \Rightarrow \1\1, \2\0 \Rightarrow \1\1\}$ over the alphabet $\{\0,\1,\2\}$ from Example~\ref{xmp:H}, and terms $N := r_1\,p_2\,(r_2\,p_3\,(r_3\,p_1\,z_\1))$ and
$M := z_\star\,p_1\,(\lambda p_2.z_\star\,p_2\,(\lambda p_3.z_\0\,p_3\,N))$.

we have that (replacing $G_1^i$ and $G_2^i$ accordingly for $i \in \{0, \ldots, 4\}$) \\
...
In accordance with Lemma~\ref{lem:Q_ssts_sem} we have that $\0^3 \Rightarrow_\frakR^* \1^3$.
...
First, by induction on the number of rewriting steps construct a term $N \in \calQ_n$ (easy converse of Lemma~\ref{lem:Q_ssts_sem}).
Second, by induction on $n$ define a term $M' \in \calR_1$ containing $N \in \calQ_n$ as a subterm (easy converse of Lemma~\ref{lem:R_ssts_sem}).
...
\end{array}$
\item $N_\frakR := \lambda y.y\,(\lambda u.u)\,\pi_\$\,\pi_\$\,\pi_\1\,\pi_\0$
