...
\newcommand{\caseelse}[3]{\ensuremath{\textbf{case } {#1} \textbf{ of } \langle #3 \mid #2 \rangle}}
...
\begin{itemize}
\item fix the constant $K \geq 6$ (in code \lstinline|Notation n := (symbols + 6)|)
\item $\bot := 0$, $\top := 1$, $\bullet := 2$, $\$ := 3$, $\0 := 4$, $\1 := 5$.
in de Bruijn -- actually reversed\\
...
\begin{definition}
$i_1, \ldots, i_k \in \{1, \ldots, K\}$ are distinct.
\[
...
\begin{lemma}
Example?\\
\[\Big(\lambda r_1 \ldots r_K.\itb{\pi_i}{j}{M}\Big) =_\beta \begin{cases}
\lambda r_1 \ldots r_K.M & \text{if } i = j\\
\pi_K & \text{if } i \neq j
\end{cases}\]
\end{lemma}
...
For $m, k \in \bbN$ let $\calQ_m^k$ and $\calR_m^k$ be the smallest sets of terms satisfying the following rules.
\begin{itemize}
\setlength\itemsep{\isep}
\item $z_\1 \in \calQ_m^k$
\item $M \in \calQ_m^k \implies (r_i\,p_j\,M) \in \calQ_m^k$ for $i \in \{1, \ldots, k\}$ and $j \in \{1, \ldots, m\}$
\item $M \in \calQ_m^k \implies (r_i\,(\lambda w.p_j\,w)\,M) \in \calQ_m^k$ for $i \in \{1, \ldots, k\}$ and $j \in \{1, \ldots, m\}$
\item $M \in \calQ_m^k \implies (z_\0\,N\,M) \in \calR_m^k$
\item $M \in \calR_{m+1}^k \implies (z_\star\,N\,(\lambda p_{m+1}.M)) \in \calR_m^k$
\end{itemize}
\end{definition}

\begin{remark}
Terms in $\calR_0$ are of shape used by Dudenhefner in Urzyczyn inhabitation.
Terms in $\calQ_m^k$ are computations of ssts in width $m$ and $k$ rules.
\end{remark}
...
&r_1 : (\kappa \to \kappa) \to \kappa \to \kappa, \ldots, r_k : (\kappa \to \kappa) \to \kappa \to \kappa\} \text{ for } k, m \in \bbN
\end{aligned}
\end{align*}
\end{definition}
...
\item $\emptyset \vdash \delta_i : \Gamma_m^k(p_j)$ for $i \in \{1, \ldots, K\}$ and $j \in \{1, \ldots, m\}$
\item $\emptyset \vdash H_\star : \Gamma_m^k(z_\star)$
\item $\emptyset \vdash H_\0 : \Gamma_m^k(z_\0)$
\item $\emptyset \vdash H_\1 : \Gamma_m^k(z_\1)$
\item $\emptyset \vdash H_R : \Gamma_m^k(r_i)$ for $i \in \{1, \ldots, k\}$
\end{itemize}
\end{lemma}
...
%  ring2 m M.
For $k, m \in \bbN$, if $M$ is in normal form such that $\Gamma^k_m \vdash M : \kappa$,\\
$\begin{array}{lllllllll}
M[z_\star := \lambda h.\lambda g.g\,I, &z_\1 := u, &z_\0 := \lambda h.I, &r_1 := I, \ldots, &r_k := I, &p_1 := I, \ldots, &p_m := I] &=_\beta u,\\
M[z_\star := H_\star, &z_\1 := \pi_\1, &z_\0 := H_\0, &r_1 := H_R, \ldots &r_k := H_R, &p_1 := \delta_\bullet, \ldots, &p_m := \delta_\bullet] &=_\beta \pi_{\1},
\end{array}$\\
then $M \in \calQ_m^k$.
\end{lemma}
...
$\begin{array}{lllllllll}
M[z_\star := \lambda h.\lambda g.g\,I, &z_\1 := u, &z_\0 := \lambda h.I, &r_1 := I, \ldots, &r_k := I, &p_1 := I, \ldots, &p_m := I] &=_\beta u,\\
M[z_\star := H_\star, &z_\1 := \pi_\1, &z_\0 := H_\0, &r_1 := H_R, \ldots &r_k := H_R, &p_1 := \delta_\bullet, \ldots, &p_m := \delta_\bullet] &=_\beta \pi_{\$},
\end{array}$\\
then $M \in \calR_m^k$.
\end{lemma}
...
For $k \in \bbN$, if $M$ is in normal form such that\\$\emptyset \vdash M : \Gamma_1^k(r_1) \to \cdots \to \Gamma_1^k(r_k) \to \Gamma_1^k(z_\0) \to \Gamma_1^k(z_\1) \to \Gamma_1^k(z_\star) \to \Gamma_1^k(p_1) \to \kappa$,\\
$\begin{array}{ll}
M\,I \ldots I\,(\lambda h.I)\,u\,(\lambda h.\lambda g.g\,I)\,I &=_\beta u,\\
M\,H_R \ldots H_R\,H_\0\,H_\1\,H_\star\,\delta_\bullet &=_\beta \pi_{\$},
\end{array}$\\
then $M = \lambda r_1 \ldots r_k.\lambda z_\0 z_\1 z_\star p_1.N$ for some $k \in \bbN$ such that $N \in \calR_1^k$.
\end{theorem}
...
\item $G_\1$ realizes the element $\1$.
\end{itemize}
\end{remark}
...
\begin{itemize}
\item $M \in \calR_m$
\item $\Gamma_\kappa \vdash M : \kappa$
\item $M[0] =_\beta \pi_\$$
\item $M[1] =_\beta \pi_\1$
\item $M[2+i] =_\beta \pi_\0$
\end{itemize}
Then, exists $N \in \calQ_{k+m}$ and
\begin{itemize}
\item $\Gamma_\kappa \vdash N : \kappa$
\item $N[0] =_\beta \pi_\1$
\item $N[1] =_\beta \pi_\0$
\item $N[2+i] =_\beta \pi_\0$
\end{itemize} 
\end{lemma}
...
\item $M \in \calR_m$
\item $\Gamma_\kappa \vdash M : \kappa$
\item $M[0] =_\beta \pi_\1$
\item $M[1+i] =_\beta \pi_\0$
\end{itemize}
Then, $\0^{1+m}$ rewrites to $\1^{1+m}$
\end{lemma}

\begin{proof}[Proof\;\coq{L2106}.]
Generalizing over the particular word $\0^{1+m}$, by induction on the size of $M$ and exhaustive case analysis using Definition~\ref{def:ring}.
\end{proof}

\begin{theorem}
Exists $n$ such that $\0^n $ rewrites to $\1^n$ iff
there exists $M$ is in normal form, such that the following conditions hold,\\
$\emptyset \vdash M : ???$\\
$\begin{array}{ll}
M\,I \ldots I\,(\lambda h.I)\,u\,(\lambda h.\lambda g.g\,I)\,I &=_\beta u,\\
M\,H_R \ldots H_R\,H_\0\,H_\1\,H_\star\,\delta_\bullet &=_\beta \pi_{\$},\\
