...
Given a simple semi-Thue system $\frakR$, one can construct simply typed terms $\emptyset \vdash F_\frakR : \sigma_\frakR \to \tau_\frakR$ and $\emptyset \vdash N_\frakR : \tau_\frakR$, a finite function $\calF_\frakR$, and an intersection type $T_\frakR$ such that the following statements are equivalent:
\begin{enumerate}
...
Of course, $(1) \Rightarrow (3)$ can be concluded from undecidability of $\lambda$-definability~\cite{loader2001undecidability} and $(1) \Rightarrow (4)$ from undecidability of intersection type inhabitation~\cite{Urzyczyn09}.
The presented approach shows $(1) \Rightarrow (2)$, and we make the following two observations.
First, based on Remark~\ref{rem:G}, the approach is easily adapted to show $(1) \Rightarrow (3)$, such that the realizer is essentially a member of $\calR_1$.
Second, referring to Remark~\ref{rem:R1}, the approach is easily adapted to show $(1) \Rightarrow (4)$, such that the inhabitant is essentially a member of $\calR_1$.
