...
Building upon the existing work, we obtain a certified many-one reduction from the Halting Problem to higher-order $\beta$-matching.
...
The specific rewriting problem, referred to as $\0^+ \Rightarrow^* \1^+$, is: given a collection of rules of shape $ab \Rightarrow cd$, where $a,b,c,d$ are alphabet symbols, is there a non-empty sequence of $\0$s which can be transformed into a sequence of $\1$s?
...
Statements and proofs in the digital version of the present work are linked to the corresponding mechanization, marked by the symbol\;\coqextra{LambdaCalculus/HOMatching.v}.
...
Term application associates to the left, and we may group consecutive $\lambda$-abstractions.
...
Since simply typed terms are strongly normalizing and $\beta$-reduction is confluent~\cite{Barendregt13book}, it suffices to consider matching solutions in normal form.
...
Consider the terms $F := \lambda x.\lambda y.x\,y\,(\lambda z.z)$ and $N := \lambda y.y$, for which we have $\emptyset \vdash F : (\ga \to (\ga \to \ga) \to \ga) \to (\ga \to \ga)$ and $\emptyset \vdash N : \ga \to \ga$.

The matching instance ${F}\,\mathsf{X} = N$ is solvable, including the solution $M := \lambda u.\lambda f.f\,u$.
Specifically, we have the following two properties:
\begin{itemize}
...
Commonly, we encode certain functional behavior as a matching instance.
...
This solution exploits the exact representation of elements via projections, disregarding the intended meaning of the underlying representation.
\end{example}
...
\end{itemize}
The matching instance $F\,\mathsf{X} = N$ combines the matching instance from Example~\ref{xmp:ad-hoc} with the additional property from Example~\ref{xmp:no-ad-hoc}. 
...
However, ad-hoc solutions such as $\lambda f.\lambda s.\pi_3$ or $\lambda f.\lambda s.\lambda x y z.f\,s\,z\,z\,z$ from Example~\ref{xmp:ad-hoc} do not solve $F\,\mathsf{X} = N$ because such solutions contain too many $\lambda$-abstractions (cf.~Example~\ref{xmp:no-ad-hoc}).
\end{example}
...
The observation from the above Example~\ref{xmp:no-ad-hoc-combo} is generalized by Loader to encode arbitrary families of finite functions.
...
In the present work, we focus on a fragment, which can be identified by inspection of intersection types occurring in the undecidability of intersection type inhabitation~\cite{Urzyczyn09,DudenhefnerR19} and their relationship to finite model theory~\cite{SalvatiMGB12}.
...
However, Joly shifts the technical burden to a particular $\lambda$-definability problem, which then is simpler to handle.
In the present work, we avoid $\lambda$-definability altogether and use a rewriting problem in a restricted class of semi-Thue systems as a starting point.

The following Remark~\ref{rem:eta-break} shows how the addition of $\eta$-reduction would invalidate the technique from Example~\ref{xmp:no-ad-hoc-combo}, as well as Loader's approach.
...
\end{array}$\\
In particular, $\eta$-reduction allows for additional $\lambda$-abstractions in the solution, invalidating the technique from Example~\ref{xmp:no-ad-hoc}.
\end{remark}
...
Finally, we present the combination constraints from the above Lemma~\ref{lem:ssts_to_constr} as a matching instance $F_\frakR\,\mathsf{X} =_\beta N_\frakR$.
...
While the existing proofs by Loader~\cite{Loader03} and by Joly~\cite{Joly05} are based on variants of $\lambda$-definability, the presented proof reduces a rewriting problem (Problem $\0^+ \Rightarrow^* \1^+$) to higher-order $\beta$-matching.
...
As pointed out in Remark~\ref{rem:typing-break}, the presented approach might be adapted to an untyped scenario.
This would require more effort with respect to syntactic constraints (cf.\,Section~\ref{sec:undec}), and presents an interesting direction for further exploration.
