...
Undecidability of higher-order $\beta$-unification was established by Huet~\cite{Huet73} in the 1970s, raising the question whether the $\beta$-matching problem~\cite{Huet75} (only the left-hand side term $M$ is subject to instantiation) is decidable\footnote{Dowek~\cite{Dowek01} gives a comprehensive overview over unification and matching problems for the $\lambda$-calculus.}.
...
Towards this aim, a technique how to restrict the shape of solutions is shown in the following Example~\ref{xmp:no-ad-hoc}.
...
The presented approach shows $(1) \Rightarrow (2)$.
...
First, based on Remark~\ref{rem:R1}, the approach is easily adapted to show $(1) \Rightarrow (3)$, such that the inhabitant is essentially a member of $\calR_1$.
This is done in the existing mechanized reduction from Problem $\0^+ \Rightarrow_\frakR^* \1^+$ to intersection type inhabitation\;\coqextra{IntersectionTypes/Reductions/SSTS01\_to\_CD\_INH.v}.
