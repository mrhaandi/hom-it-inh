...
The present work provides a novel undecidability proof for higher-order $\beta$-matching, in an effort to verify this result by means of a proof assistant in full detail.
...
Decidability of higher-order $\beta$-matching was answered negatively\footnote{Not to be confused with the positive answer by Stirling~\cite{Stirling09} for higher-order $\beta\eta$-matching.} by Loader~\cite{Loader03} by reduction from a variant of $\lambda$-definability.
Loader's proof introduces intricate machinery to formulate $\beta$-matching constraints which specify arbitrary finite functions.
A later approach by Joly~\cite{Joly05} refines Loader's result, shifting technical challenges to undecidability of the underlying $\lambda$-definability problem.
Both approaches render verification of the negative result in full detail (for example, by means of a mechanization in a proof assistant) quite challenging.

%Second-order matching was established as decidable by Huet and Lang, providing a foundation for analyzing restricted cases of higher-order problems.
%Third-order matching was shown to be decidable by Dowek, while fourth-order matching was tackled by Padovani, who provided decision procedures for specific subclasses.
%The decisive shift occurs at higher orders. Loader’s seminal work demonstrated the undecidability of higher-order β-matching at order six, relying on intricate reductions and logical constructions. Loader’s proof extends to the encoding of definability problems in finite models, leveraging the complexity of these definability questions to establish undecidability.
%The distinction between β-equivalence and beta-eta-equivalence adds further complexity to the landscape. While beta-eta-equivalence is more robust and often considered in the context of semantic equality, the undecidability results established by Loader pertain strictly to β-equivalence, leaving the status of beta-eta-matching undecided.
...
Statements and proofs in the digital version of the present work are linked to the corresponding mechanization, which is marked by the symbol\;\coqextra{LambdaCalculus/HOMatching.v}.
...
\begin{problem}[$\0^+ \Rightarrow^* \1^+$\;\coqextra{StringRewriting/SSTS.v\#L37}]\label{prb:ssts01}
...
\begin{definition}[Sets $\calQ_m$\;\coq{L1593}, $\calR_m$\;\coq{L1600} of Terms]\label{def:ring}
...
\begin{definition}[Type Environment $\Gamma_m$\;\coq{L459}]\label{def:Gamma}
For $m > 0$ let
\begin{align*}
...
\begin{definition}[Typed Terms $H_\star$\;\coq{L597}, $H_\0$\;\coq{L598}, $H_R$\;\coq{L599}]\label{def:H}
\begin{align*}
...
\begin{definition}[Substitutions $S_F$\;\coq{L519}, $S_H$\;\coq{L602}]\label{def:SF_SH}
~\\
$\begin{array}{llll}
...
\begin{definition}[Typed Terms $G_\star$\;\coq{L794}, $G_\0$\;\coq{L812}, $G_{ab \Rightarrow cd}$\;\coq{L823}, $G_i^j$\;\coq{L788}]
\label{def:G}
\begin{align*}
...
\begin{definition}[Substitution $S_G$\;\coq{L834}]\label{def:S_G}
