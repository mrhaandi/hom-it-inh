...
Higher-order $\beta$-Matching is the following typed unification problem, for which only one side is instantiated. 
...
For simple semi-Thue systems it is undecidable whether some sequence TODO

\begin{problem}[$\0^+ \Rightarrow^*_\frakR \1^+$]\label{prb:ssts01}
...
The mechanization contributed to the existing Coq Library of Undecidability Proofs~\cite{CLUP20}.
...
Additionally, undecidability of the particular rewriting problem is already mechanized, and is part the the Coq Library of Undecidability Proofs.
...
Constraining the shape of candidate solutions in the present work (as well as in Loader's proof) seems to require order $6$.
Still, decidability of $\beta$-Matching at order $5$ remains open.
