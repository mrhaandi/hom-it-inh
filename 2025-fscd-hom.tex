...
The mechanization eliminates potential ambiguities and errors, complementing existing work on mechanized undecidability of higher-order $\beta$-unification~\cite{Spies020}.
...
The specific rewriting problem, referred to as $\0^+ \Rightarrow^*_\frakR \1^+$, is: given a collection $\frakR$ of rules of shape $ab \Rightarrow cd$, where $a,b,c,d$ are alphabet symbols, is there a non-empty sequence of $\0$s which can be transformed into a sequence of $\1$s?
...
Consider the instance $(\lambda t.t\,G\,\pi_1)\,\mathsf{X} = \pi_3$ of Higher-order $\beta$-Matching where $\emptyset \vdash \lambda t.t\,G\,\pi_1 : ((\kappa \to \kappa) \to \kappa \to \kappa) \to \kappa$.
...
In the above Example~\ref{xmp:ad-hoc} we would like to exclude certain \enquote{ad-hoc} solutions.
...
&=_\beta& \lambda xyz.\pi_1\,z\,y\,x \twoheadrightarrow_\beta \lambda xyz.z = \pi_{3}
\end{array}$\\
$\arraycolsep=1.4pt\begin{array}[t]{lclcl}
M\,(\lambda x.x)\,u &=_\beta& \big(\lambda g.\lambda h.\lambda xyz.h\,(g\,\pi_1 x\,z\,y)\,y\,z\big)\,(\lambda x.x)\,u
&=_\beta& \lambda xyz.u\,((\lambda x.x)\,\pi_1 x\,z\,y)\,y\,z\\
...
Additionally, undecidability of Problem $\0^+ \Rightarrow_\mathfrak{R}^* \1^+$ is already mechanized, and is part the Coq Library of Undecidability Proofs.
