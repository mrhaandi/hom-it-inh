...
Higher-order $\beta$-unification in the simply typed $\lambda$-calculus is the following decision problem: given two simply typed $\lambda$-terms $M, N$, is there a substitution $S$ such that the term $S(M)$ is $\beta$-equivalent to the term $S(N)$?
Undecidability of higher-order $\beta$-unification was established by Huet~\cite{Huet73} in the 1970s, raising the question whether the corresponding $\beta$-matching problem~\cite{Huet75} (only the left-hand side term $M$ is subject to instantiation) is decidable\footnote{Dowek~\cite{Dowek01} gives a comprehensive overview over unification and matching problems for the $\lambda$-calculus.}.
...
Loader's proof introduces intricate machinery to encode arbitrary finite functions as higher-order $\beta$-matching.
...
rather, we consider a rewriting problem in a restricted class of semi-Thue systems~\cite[Lemma~2]{Urzyczyn09} as a starting point.
...
Urzyczyn's undecidability proof for intersection type inhabitation~\cite{Urzyczyn09} serves as the main inspiration for the novel approach in the present work.
Considering the relationship between intersection type inhabitation and finite model theory~\cite{SalvatiMGB12}, the approach in the present work has an additional benefit:
it is uniformly applicable to higher-order $\beta$-matching, intersection type inhabitation, and $\lambda$-definability.
...
\begin{itemize}
\item $G_\star$ realizes a member of the finite function family ${\small\left(\begin{array}{l}
...
\end{array}\right)}$.
\item $G_\0$ realizes a member of the finite function family ${\small\left(\begin{array}{l}
...
\end{array}\right)}$.
\item $G_{ab \Rightarrow cd}$ realizes a member of the finite function family ${\small\left(\begin{array}{l}
